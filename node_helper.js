...
module.exports = NodeHelper.create({
  ...
  getWX: function (payload) {
    ...
    //var airports = payload.airportList.split(",");
    ...
    request({url: metarUrl,method: "GET",gzip: true,
    })
      .pipe(zlib.createGunzip())
      .pipe(concat(stringBuffer => {

        let json_string = convert.toJson(stringBuffer.toString())
        var json = JSON.parse(json_string);
        var metarData = json.response.data.METAR

        if (this.debug) {
          //console.log("json=" + JSON.stringify(metarData, null, 2))
          console.log("test data as json="+JSON.stringify(JSON.parse(convert.toJson(test_data)),null,2))
        }
        ...
        request({ url: FAAUrl, method: "GET" })
          .pipe(concat(stringBuffer => {
            //console.log(stringBuffer.toString());
            //console.log("fetch response= " + JSON.stringify(stringBuffer))
            let json_string =
              //convert.toJson(test_data)     swap comments to use test FAA data
              convert.toJson(stringBuffer.toString())
            if(this.debug)
